...
replacfun <- function(mat,protracted_mat,sd,speciation_rate){    
  ...
  if(spec < speciation_rate){
    ...
    protracted_mat <- speciation_fun(protracted_mat,mat,speciation_rate,deadx,deady)
  }
...
multigensp <- function(time,mat,sd,speciation_rate,speciation_length,file_name,filename_mat){
  ...
  new_count <- vector()
  initial_values <- unique(as.vector(mat))
  ...
  while (!initialization_done) {
    ...
    remaining_initials <- intersect(initial_values, unique(as.vector(mat)))
    if (length(remaining_initials) == 0) {
  ...
  for(k in sp1){
    name <- names(first)
    index <- which(name == k)
    x[[as.character(k)]][1]<- first[index]
  }
  for(i in 1:time){
    for(j in 1:(length(mat)/2)){
      result <- replacfun(mat,pro_mat,sd,speciation_rate)
    ...
    for(k in sp){
      name <- names(y)
      index <- which(name == k)
      x[[as.character(k)]][i]<- y[index]
    }
    sp1 <- sp
  }
  x <- rm_sp(x)
  #write.csv(as.data.frame(do.call(cbind,x)),file = file_name)
  #save(mat,pro_mat,file = filename_mat)
  return(x)
  }

start.time <- Sys.time()
 result <- multigensp(100,matrix_start(100,100,10),4,0.003,10)
end.time <- Sys.time()
